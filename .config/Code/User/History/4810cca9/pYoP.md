...
This is optional but it is useful that if the minecraft crashes that it will auto restart and that your server will automatically start after the machine it is running on has booted up.
```bash
systemctl --user enable mc@<server-name>
```
where `<server-name>` is the name of a sub-dir in `~/minecraft` where you will be running a server.