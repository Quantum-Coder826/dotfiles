...
path = "/home/berend/Pictures/screenshot_" + str(time.strftime("%Y-%m %H-%M-%S")) + ".png"
#path.touch(exist_ok=True)

with mss() as sct:
    filname = sct.shot(mon=screen, output=path) # take screenshot and save it

system(f"xclip -selection clipboard -t image/png -i {path + '.png'}") # send screenshot to clipboard with xclip
system(f"notify-send screenshot was made and copyed to clipboard")