...
fn main() {
    ...
    for line in source_list {
        ...
        let operand: String  = match iter_line.next().unwrap() {
            Some("ADD") => "0".to_string(),
            Some(None) => panic!("Tried to decode unknown instruction '{}' at line: {}", iter_line.next_back().unwrap().to_string(), source_address + 1)
        };
