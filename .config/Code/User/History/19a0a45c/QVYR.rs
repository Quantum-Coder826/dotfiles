...
pub struct Brick {
    thread_key_watcher: JoinHandle<()>,
    thread_run_bool: Mutex<bool>,
}
...
impl Brick {
    pub fn init() -> Brick {
        ...
        let thread_run_bool: Mutex<bool> = Mutex::new(true);

        let thread_key_wacher:JoinHandle<()> = thread::spawn(move || {
        ...
        Brick {
            thread_key_watcher: thread_key_wacher,
            thread_run_bool: thread_run_bool
        }
    }
    ...
    pub fn exit(self) {
        Self::reset();
        self.thread_key_watcher.join().unwrap();
        exit(0);
    }
