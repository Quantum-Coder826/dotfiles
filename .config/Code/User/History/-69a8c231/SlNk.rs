...
fn main() {
    ...
    loop {
        ...
        match instruction_register.parse::<u8>().unwrap() {
            ...
            2 => {
                // SUB
                let number: i16 = (memory[address_register.parse::<usize>().unwrap()]).parse::<i16>().unwrap(); // get value for subtract instruction
            ...
            3 => {
                // STA
                memory[address_register.parse::<usize>().unwrap()] = accumulator.to_string().clone();
            },
            4 => {
                // LDA
                let value: i16 = (memory[address_register.parse::<usize>().unwrap()]).parse::<i16>().unwrap(); // get value from mailbox for insturction
                accumulator = value.clone();
            },
            5 => {
                // BRA
                program_counter = address_register.parse::<usize>().unwrap().clone() - 1; // pc - 1 due to programcounter increment at top of program
            },
            6 => {
                // BRZ
                if accumulator == 0 {
                    program_counter = address_register.parse::<usize>().unwrap().clone() - 1;
                    continue;
                }
            },
            7 => {
            ...
            0 => {
                if address_register == "00".to_string() {
                    println!("HALT");
                    exit(0);
                }
            }
