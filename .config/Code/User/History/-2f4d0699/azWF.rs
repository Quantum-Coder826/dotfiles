...
use std::{fs::read_dir, iter::Enumerate};

enum SensorReturn<I, F> {
    Int(I),
    Float(F)
}
pub struct Sensor {
...
impl Sensor {
    ...
    pub fn get_value(self, value: u8) -> SensorReturn<i64, f64>{
        let raw_data: i64 = files::read_int(&(self.path.clone() + "/value" + &value.to_string()));
        if self.decimals == 0 { 
            return raw_data as i64;
        } else {
            let mut i:u64 = 0;
            let mut data:f64 = raw_data as f64;
