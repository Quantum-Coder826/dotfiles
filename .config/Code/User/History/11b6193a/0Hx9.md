...
### ```init()```
Init initializes the brick it will set both green led's to full brightness and send the reset command to all connected motors. Also is adds a exit handeler what will do the same steps but on 
