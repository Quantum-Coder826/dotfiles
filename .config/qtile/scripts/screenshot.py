#!/usr/bin/env python3

from libqtile.command.client import InteractiveCommandClient
from mss.linux import MSS as mss
from datetime import datetime
from sys import argv
from os import system
from shlex import quote

c = InteractiveCommandClient()

time = datetime.now() # get current date and time

# this variable is used to set wich screen is captured
# -1 for all screens
# 1 for main monitor
# 2 for secondairy

if len(argv) <= 1:
    screen = c.group.info()["screen"] # get the current active screen from qtile (zero base index)
    screen += 1 # turn the qtile screenindex from a zero base to a one base index
else:
    screen = int(argv[1])

path = "/home/berend/Pictures/screenshot_" + str(time.strftime("%Y-%m-%d %H-%M-%S")) + ".png"

with mss() as sct:
    filname = sct.shot(mon=screen, output=path) # take screenshot and save it

print(path)
system("xclip -selection clipboard -t image/png -i {}".format(quote(path))) # send screenshot to clipboard with xclip
system(f"notify-send 'screenshot was made and copyed to clipboard'") # send a notification